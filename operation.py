...
def check_bin_exist() -> OperationResult:
    ...
    def check_correct(process: subprocess.Popen, output: str, index: int):
        ...
        if (output.startswith('nwipe version ')):
            ...
            result.message = output
    process, thread = __open_pipe_with_multi_process(
        ['sudo', '-S', NWIPE_BIN_PATH, '--version'], check_correct)
    if __is_need_sudo():
        process.stdin.write(str(SUDO_PASSWD).encode() + b'\n')
        process.stdin.close()
    returncode = __close_pipe_and_wait(process, thread, True)
...
def run_nwipe(
        ...
        verify: Union[Verifies.off, Verifies.last,
                      Verifies.all] = Verifies.last,
        method: Union[Methods.dod522022m, Methods.dodshort, Methods.gutmann, Methods.ops2, Methods.random, Methods.prng,
                      Methods.stream, Methods.zero, Methods.quick, Methods.one, Methods.verify_zero, Methods.verify_one] = Methods.dodshort,
        rounds: int = 1,
        log_path: str = None,
